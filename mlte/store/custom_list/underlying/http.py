...
from mlte.store.custom_list.store_session import CustomListEntryMapper, CustomListStoreSession
...
class HttpCustomListEntryMapper(CustomListEntryMapper):
    ...
    def create(self, new_entry: CustomListEntryModel, list_name: Optional[CustomListName] = None) -> CustomListEntryModel:
        response = self.storage.post(json=new_entry.to_json(), groups=_list_group(list_name))
        return CustomListEntryModel(**response)

    def read(self, entry_name: str, list_name: Optional[CustomListName] = None) -> CustomListEntryModel:
        response = self.storage.get(id=entry_name, groups=_list_group(list_name))
        return CustomListEntryModel(**response)
    ...
    def edit(self, updated_entry: CustomListEntryModel, list_name: Optional[CustomListName] = None) -> CustomListEntryModel:
        response = self.storage.put(json=updated_entry.to_json(), groups=_list_group(list_name))
        return CustomListEntryModel(**response)

    def delete(self, entry_name: str, list_name: Optional[CustomListName] = None) -> CustomListEntryModel:
        response = self.storage.delete(id=entry_name, groups=_list_group(list_name))
        return CustomListEntryModel(**response)
