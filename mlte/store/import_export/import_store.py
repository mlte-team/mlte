        ...
        for list_name in custom_list_data.keys():
            if list_name not in list(map(str, CustomListName)):
                raise errors.ErrorNotFound(
                    f"CustomListName {list_name} does not exist."
                )
            ...
            for entry in custom_list_data[list_name]:
                if entry["name"] in entry_id_list:
                    raise errors.ErrorAlreadyExists(
                        f"Entry {entry['name']} in Custom List {list_name}"
                    )
        ...
        with ManagedCatalogSession(
            ...
            for entry in catalogs_data[catalog_name]:
                ...
                if entry_id in entry_list:
                    raise errors.ErrorAlreadyExists(
                        f"Entry {entry_id} in catalog {catalog_name}"
                    )
            ...
            for version_id in artifact_data[model_id].keys():
                ...
                for artifact_id in artifact_data[model_id][version_id].keys():
                    if artifact_id not in artifact_id_list:
                        artifact_store_session.artifact_mapper.create(
                            ArtifactModel(
                                **artifact_data[model_id][version_id][
                                    artifact_id
                                ]
                            ),
                            (model_id, version_id),
                    ...
                    if artifact_id in artifact_id_list and force:
                        artifact_store_session.artifact_mapper.edit(
                            ArtifactModel(
                                **artifact_data[model_id][version_id][
                                    artifact_id
                                ]
        ...
        for list_name in custom_list_data.keys():
            ...
            for entry in custom_list_data[list_name]:
                if entry["name"] not in entry_id_list:
                    custom_list_store_session.custom_list_entry_mapper.create(
                        CustomListEntryModel(**entry), list_name
                    )
                elif entry["name"] in entry_id_list and force:
                    custom_list_store_session.custom_list_entry_mapper.edit(
                        CustomListEntryModel(**entry), list_name
                    )


def _import_users(
    user_data: dict[str, Any], user_store: UserStore, force: bool = False
) -> None:
        ...
        with ManagedCatalogSession(
            ...
            for entry in catalogs_data[catalog_name]:
                ...
                if entry_id not in entry_list:
                    catalog_store_session.entry_mapper.create(
                        CatalogEntry(**entry)
                    )
                elif entry_id in entry_list and force:
                    catalog_store_session.entry_mapper.edit(
                        CatalogEntry(**entry)
                    )
