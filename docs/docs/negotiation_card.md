...
An ID to identify the quality attribute scenario. This is generated by the backend upon saving the negotiation card. It is used to link this scenario to test cases in the test suite.

### System Quality

System property by which the model will be evaluated, e.g., accuracy, performance, robustness, fairness, resource consumption. The selection is split into two. First selecting the quality attribute category and then selecting the quality attribute. These selections are currently a hard-coded list within the package but will be able to be edited by the user in a future update.
