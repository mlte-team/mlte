...
`MLTE` uses quality attribute scenarios as a way to specify a system’s structural and behavioral requirements. To read more about quality attriute scenarios, see this <a href="https://doi.org/10.48550/arXiv.2406.08575" target="_blank">paper</a>. Below are resources on potential attributes for three categories: functionality, robustness, and costs, as well as some other machine learning topics. These resources are not comprehensive but may be of interest to teams using `MLTE`.
...
## Model Measurement Definition

Research on model measurement definition.
