...
After building the `TestSuite`, `MLTE` requires developers to collect evidence to attest to whether or not the model realizes its stated goals. `Evidence` is generated by the `Measurement`s which are specified in each `TestCase` as described [above](#1-define-a-testsuite). Each invididual piece of data produced by running a `TestCase` is an `Evidence`. Once `Evidence` is produced, we can persist each one to an artifact store to maintain them across sessions.

Here is an example of collecting `Evidence` for a `TestCase` on accuracy:
...
To see more examples of `Evidence` collection, please refer to the simple and scenario demos which can be found in the `MLTE` <a href="https://github.com/mlte-team/mlte/tree/master/demo" target="_blank">demo folder</a>.
