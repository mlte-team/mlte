...
![Version Badge](https://img.shields.io/badge/release-v2.5.0-e19b38)
...
Optionally a current user can also be set via ``set_credentials("user", "password")``, which can be imported from ``mlte.session`` as well. It will be used as the author of the created/edited artifacts. This can also be set by environment variables before starting the script (``MLTE_CURRENT_USER``).
