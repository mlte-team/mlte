...
from mlte.store.import_export.constants import MODELS_KEY
from mlte.store.import_export.import_store import _import_artifacts, _import_catalogs, _import_custom_lists, _import_users, import_store
...
from test.store.import_export.conftest import ALL_EXPORT_DATA, CATALOG_EXPORT_DATA, CUSTOM_LIST_EXPORT_DATA, USER_EXPORT_DATA
...
def test_import_artifacts(
    ...
    with ManagedArtifactSession(
        ...
        assert artifact == artifact_store_session.artifact_mapper.read(f"card.{artifact_id}", (model_id, version_id))
...
def test_import_custom_lists(
    ...
    with ManagedCustomListSession(
        ...
        assert CustomListEntryModel(**entry) == custom_list_store_session.custom_list_entry_mapper.read(entry["name"], list_name)
...
def test_import_catalogs(
    ...
    with ManagedCatalogSession(
        ...
        assert CatalogEntry(**entry) == catalog_store_session.entry_mapper.read(entry["header"]["identifier"])
